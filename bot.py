...
RICH_MESSAGES_FILE = "rich_messages_log.json"
RICH_MESSAGES_KEEP = int((os.environ.get("RICH_MESSAGES_KEEP") or "200").strip())
...
# ── Rich Message Log (message_id для редактирования) ─────────────────────────

def load_rich_messages_log() -> list:
    """Загружает лог опубликованных Rich-постов из JSON-файла."""
    path = Path(RICH_MESSAGES_FILE)
    if not path.exists():
        return []
    try:
        with open(path, encoding="utf-8") as f:
        ...
        print(f"  Не удалось прочитать {RICH_MESSAGES_FILE}: {e}")
        return []


def save_rich_messages_log(records: list):
    """Сохраняет лог Rich-постов, обрезая до RICH_MESSAGES_KEEP последних записей."""
    records = records[-RICH_MESSAGES_KEEP:]
    with open(RICH_MESSAGES_FILE, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=2)


def log_rich_message(message_id: int, html_body: str, post_id: str = ""):
    """Добавляет запись об опубликованном Rich-посте в лог."""
    records = load_rich_messages_log()
    records.append({
    ...
    save_rich_messages_log(records)
    print(f"  Rich message_id={message_id} сохранён в {RICH_MESSAGES_FILE}")
...
def telegram_html_to_rich_html(text: str) -> str:
    ...
    Маркированные списки (•, -, *) → <ul><li>, нумерованные (1., 2.) → <ol><li>.
    """
    text = (text or "").strip()
    if not text:
        return ""
    ...
    for para in paragraphs:
        ...
        # Проверяем — весь абзац это маркированный список?
        if all(bullet_re.match(l) for l in lines if l.strip()):
            items = "".join(
                f"<li>{bullet_re.match(l).group(1).strip()}</li>"
                for l in lines if l.strip()
            )
            parts.append(f"<ul>{items}</ul>")
            continue

        # Проверяем — весь абзац это нумерованный список?
        if all(ordered_re.match(l) for l in lines if l.strip()):
            items = "".join(
                f"<li>{ordered_re.match(l).group(1).strip()}</li>"
                for l in lines if l.strip()
            )
...
def _apply_emoji_enhancer(rich_html: str) -> str:
    """
    Обёртка над enhance_rich_emoji из emoji_registry.py.
    Если файл не найден или emoji-id не заполнены — возвращает html без изменений.
    Вызывается в конце build_rich_layout перед отдачей в send_rich_post.
    """
    try:
        from emoji_registry import enhance_rich_emoji
        return enhance_rich_emoji(rich_html)
    except ImportError:
        pass  # emoji_registry.py не подключён — ок, работаем без кастомных эмодзи
    except Exception as e:
        print(f"  _apply_emoji_enhancer ошибка: {e}")
    return rich_html
...
5. Маркированный список (•, -, *) → <ul><li>...</li>...</ul>
6. Нумерованный список (1., 2., ...) → <ol><li>...</li>...</ol>
7. Табличные данные (несколько колонок, статистика) → <table><tr><td>...</td></tr></table>
8. В конце добавь ТОЧНО эту строку подписи без изменений: <p>{sig_html}</p>
    ...
    try:
        ...
        if rich_html and "<p>" in rich_html:
            ...
            if _SIG_URL and _SIG_URL not in rich_html:
                rich_html += f"\n<p>{sig_html}</p>"
            return _apply_emoji_enhancer(rich_html)
    except Exception as e:
        print(f"  build_rich_layout ошибка: {e}")

    # Fallback: детерминистическая конвертация + подпись с HTML-ссылкой
    fallback_html = telegram_html_to_rich_html(text) + f"\n<p>{sig_html}</p>"
    return _apply_emoji_enhancer(fallback_html)
...
def _build_rich_message_html(full_text: str, photo_file_ids: list, video_file_ids: list) -> str:
    ...
    full_text — уже готовый Rich HTML из build_rich_layout():
      - абзацы:  <p>...</p>
      - цитаты:  <blockquote>...</blockquote>
      - списки:  <ul><li>...</li></ul> / <ol><li>...</li></ol>
      - таблицы: <table><tr><td>...</td></tr></table>
      - инлайн:  <b>, <i>, <u>, <s>, <a href="...">, <tg-spoiler>
      - подпись: <p>#Хештег | <a href="...">Follow us</a></p>
    Передаётся как есть, без дополнительной обработки.
...
def send_rich_post(downloaded: list, full_text: str, post_id: str = "") -> bool:
    """
    ...
    try:
        ...
        if not res.get("ok"):
            ...
            if msg_id:
                log_rich_message(msg_id, html_body, post_id=post_id)
        return res.get("ok", False)
    except Exception as e:
...
def edit_rich_post(message_id: int, new_text: str) -> bool:
    """
    Редактирует уже опубликованный Rich-пост через editRichMessage (Bot API 10.1+).

    new_text — plain текст или Telegram HTML. Функция прогоняет его через
    build_rich_layout (Gemini + SIGNATURE) и отправляет editRichMessage.

    Медиа не меняется — только текстовая часть Rich-сообщения.
    """
    print(f"  Редактирование Rich-поста message_id={message_id} ...")
    rich_html = build_rich_layout(new_text)

    try:
        res = requests.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/editRichMessage",
            json={
                "chat_id": CHANNEL_ID,
                "message_id": message_id,
        ...
        if res.get("ok"):
            print(f"  editRichMessage OK: message_id={message_id}")
            # Обновляем превью в логе
            records = load_rich_messages_log()
            for r in records:
                if r.get("message_id") == message_id:
                    r["html_preview"] = rich_html[:200]
                    r["edited_at"] = datetime.now(timezone.utc).isoformat()
                    break
            save_rich_messages_log(records)
        else:
            print(f"  editRichMessage ошибка: {res.get('description')}")
        return res.get("ok", False)
    except Exception as e:
        print(f"  edit_rich_post исключение: {e}")
        return False


def list_rich_posts(last_n: int = 20):
    """Выводит последние N опубликованных Rich-постов из лога."""
    records = load_rich_messages_log()
    if not records:
        print("  Лог Rich-постов пуст.")
        return
    for r in records[-last_n:]:
        msg_id = r.get("message_id", "?")
        ts = r.get("published_at", "")[:16]
        edited = f" [отред. {r['edited_at'][:16]}]" if r.get("edited_at") else ""
        preview = r.get("html_preview", "")[:80].replace("\n", " ")
        print(f"  [{ts}{edited}] message_id={msg_id}  {preview}")
...
def main():
    ...
    parser.add_argument("--step", type=int, choices=[1, 2, 3, 4],
                        help="Шаг парсинга (1–4)")
    parser.add_argument("--edit-rich", type=int, metavar="MESSAGE_ID",
                        help="Отредактировать Rich-пост по message_id")
    parser.add_argument("--edit-text", type=str, metavar="TEXT",
                        help="Новый текст для --edit-rich (Telegram HTML или plain)")
    parser.add_argument("--list-rich", action="store_true",
                        help="Показать последние опубликованные Rich-посты с message_id")
    parser.add_argument("--list-rich-n", type=int, default=20, metavar="N",
                        help="Сколько последних Rich-постов показать (по умолчанию 20)")
    args = parser.parse_args()

    # ── Режим просмотра лога Rich-постов ──────────────────────────────────────
    if args.list_rich:
        print(f"▶ Последние Rich-посты | {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
        list_rich_posts(last_n=args.list_rich_n)
        print("\n✓ Готово.")
        return

    # ── Режим редактирования Rich-поста ───────────────────────────────────────
    if args.edit_rich is not None:
        if not args.edit_text:
            parser.error("--edit-rich требует --edit-text с новым текстом поста")
        print(f"▶ Редактирование Rich-поста | {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
        ok = edit_rich_post(args.edit_rich, args.edit_text)
        print("\n✓ Готово." if ok else "\n✗ Ошибка редактирования.")
        return

    # ── Обычный режим: шаги парсинга ──────────────────────────────────────────
    if args.step is None:
        parser.error("Укажите --step (1–4), --edit-rich или --list-rich")
