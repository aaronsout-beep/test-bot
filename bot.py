...
def normalize_nitter_media_url(raw_url: str, page_url: str) -> str:
    ...
    # Для картинок Twitter query часто содержит только размер. Telegram/requests
    # надежнее забирают оригинальный CDN URL без nitter-прокси.
...
def fetch_instagram_posts(username: str, published: set) -> list:
    """
    instaloader читает все медиафайлы из поста, включая карусели.
    Для публичных аккаунтов обычно работает без авторизации, но Instagram может
    временно ограничивать запросы из GitHub Actions.
    """
    ...
    if importlib.util.find_spec("instaloader") is None:
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "instaloader", "--quiet"],
            check=True,
        )
