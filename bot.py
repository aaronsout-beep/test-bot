...
STYLE_EDIT_MODEL = (os.environ.get("STYLE_EDIT_MODEL") or "gemini-2.5-flash-lite").strip()
GLOSSARY_EDIT_MODEL = (os.environ.get("GLOSSARY_EDIT_MODEL") or "gemini-2.5-flash-lite").strip()
...
def translate_openrouter(text: str) -> str:
    """
    Перевод через OpenRouter — резервный метод.
    При ошибке или отсутствии OPENROUTER_KEY возвращает оригинальный текст.
    """
    if not text:
        return text
    if not OPENROUTER_KEY:
        print("  OpenRouter перевод недоступен: OPENROUTER_KEY не задан — возвращаем оригинал")
        return text
    ...
    try:
        ...
        print(f"  OpenRouter перевод ошибка: {e} — возвращаем оригинал")
        return text


def translate_gemini(text: str) -> str:
    """
    Перевод через Gemini — основной метод.
    При ошибке или недоступности GEMINI_API_KEY делает fallback на OpenRouter.
    """
    if not text:
        return text
    if not GEMINI_API_KEY:
        print("  Gemini перевод недоступен: GEMINI_API_KEY не задан — fallback на OpenRouter")
        return translate_openrouter(text)
    ...
    try:
        ...
        print(f"  Gemini перевод ошибка: {e} — fallback на OpenRouter")
        return translate_openrouter(text)


def translate_yandex(text: str) -> str:
    """
    Перевод через YandexGPT Pro — альтернативный метод.
    При недоступности Yandex API автоматически откатывается на Gemini.

    YandexGPT используется вместо Gemini + edit_style_gemini:
    ...
    if not YANDEX_API_KEY or not YANDEX_FOLDER_ID:
        print("  YandexGPT недоступен: YANDEX_API_KEY/YANDEX_FOLDER_ID не заданы — fallback на Gemini")
    ...
    try:
        ...
        status = e.response.status_code if e.response is not None else "?"
        print(f"  YandexGPT перевод HTTP {status}: {e} — fallback на Gemini")
        return translate_gemini(text)
    except Exception as e:
        print(f"  YandexGPT перевод ошибка: {e} — fallback на Gemini")
        return translate_gemini(text)
...
def edit_translation_text(text: str) -> str:
    ...
    Цепочка: локальные замены → edit_gemini (глоссарий) → edit_style_gemini (стиль)
    ...
    for item in downloaded[:TELEGRAM_MEDIA_GROUP_LIMIT]:
        ...
        try:
            ...
            with open(path, "rb") as f:
                res = requests.post(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",
                    data={"chat_id": upload_chat, "disable_notification": True},
                    files={field: f},
                    timeout=90,
                ).json()
            if not res.get("ok"):
...
def step1_twitter_keywords(published: set) -> set:
    ...
    for tw in tweets:
        ...
        translated = translate_gemini(raw)
...
def step2_instagram(published: set) -> set:
    ...
    for post in posts[:10]:
        ...
        if raw_caption:
            translated = translate_gemini(raw_caption)
            edited = edit_openrouter(translated)
        else:
            edited = ""
...
def step4_instagram_hashtags(published: set) -> set:
    ...
    for hashtag in STEP4_HASHTAGS:
        ...
        for post in posts:
            ...
            translated = translate_gemini(raw_caption) if raw_caption else ""
