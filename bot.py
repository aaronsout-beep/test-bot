...
def _vk_upload_photo(path: Path) -> str | None:
    ...
    try:
        ...
        print(f" VK upload photo url получен: {upload_url}")
        ...
        print(f" VK saveWallPhoto response: {save_r.get('response')}")
...
def post_to_vk(full_text: str, downloaded: list | None = None) -> bool:
    ...
    for item in dl_list:
        ...
        if media_type == "video":
            ...
            attach = _vk_upload_photo(path_obj)
...
def crosspost_after_telegram(full_text: str, downloaded: list | None = None):
    ...
    if _SIG_HASHTAG and not text_contains_signature_hashtag(crosspost_text):
        print(f"  Кросспостинг пропущен: #{_SIG_HASHTAG} не найден в тексте поста.")
        return
    ...
    if vk_enabled:
        valid_downloaded = [
            d for d in (downloaded or [])
            if d.get("path") and Path(d["path"]).exists()
        ]
        media_count = len(valid_downloaded)
        ...
        post_to_vk(crosspost_text, downloaded=valid_downloaded)
...
def send_to_telegram(
    ...
    Возвращает:
      (sent_ok, downloaded) — успешность отправки и список скачанных медиа
    """
    if not media_items:
    ...
    # Кросспостинг ДО удаления медиа
    if sent_ok:
        crosspost_after_telegram(final_text, downloaded=downloaded)

    # ТЕПЕРЬ удаляем медиа
    shutil.rmtree(MEDIA_DIR, ignore_errors=True)
    return sent_ok, downloaded
...
def step1_twitter_keywords(published: set) -> set:
    ...
    for tw in tweets:
        ...
        ok, _ = send_to_telegram(media_items, edited)  # _ = downloaded, не нужен здесь
        if ok:
...
def step4_instagram_hashtags(published: set) -> set:
    ...
    for hashtag in STEP4_HASHTAGS:
        ...
        for post in posts[:APIFY_HASHTAG_RESULTS_LIMIT]:
            ...
            print(f"  Публикуем ({len(media_items)} медиа): {post_id[:80]}")
            ok, _ = send_to_telegram(media_items, full_text, step4_custom_sig=True)
            if ok:
                mark_published(
                    published,
                    post_id=post_id,
