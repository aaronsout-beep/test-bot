...
# RAG-проверка верстки тоже переведена на Qwen3 235B (Yandex). Раньше здесь
...
def translate_deepl(text: str) -> str | None:
...
def translate_openrouter(text: str) -> str | None:
...
def translate_gemini(text: str) -> str | None:
...
def translate_yandex(text: str) -> str | None:
    ...
    try:
        ...
        if not _looks_translated_to_russian(text, result):
            raise ValueError("Qwen3 вернул текст не на русском (похоже, перевод не выполнен)")
        return result
    except requests.HTTPError as e:
        status = e.response.status_code if e.response is not None else "?"
...
def gemini_text_completion(
    ...
    """Вызов Gemini с автоматическим fallback на GEMINI_EDIT_FALLBACK_MODEL
    при ошибках перегрузки (503 / 429)."""
    primary_model = (model or LAYOUT_AI_MODEL).strip()
    models_to_try = [primary_model]
    if GEMINI_EDIT_FALLBACK_MODEL and GEMINI_EDIT_FALLBACK_MODEL != primary_model:
